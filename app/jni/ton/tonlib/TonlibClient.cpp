...
namespace tonlib {
    ...
    auto to_tonlib_api(const ton::BlockIdExt &blk) {
        return tonlib_api::make_object<tonlib_api::ton_blockIdExt>(
                blk.id.workchain, blk.id.shard, blk.id.seqno, blk.root_hash.as_slice().str(),
                blk.file_hash.as_slice().str());
    }

    tonlib_api::object_ptr<tonlib_api::options_configInfo>
    to_tonlib_api(const TonlibClient::FullConfig &full_config) {
    ...
    public:
        ...
        void send_query(QueryT query, td::Promise<typename QueryT::ReturnType> promise) {
            td::actor::send_lambda(client_,
                                   [self = client_.get(), query = std::move(
                                           query), promise = std::move(promise)]() mutable {
                                       self.get_actor_unsafe().make_request(std::move(query),
                                                                            std::move(promise));
                                   });
        }
    ...
    create_account_state(ton::tl_object_ptr<ton::lite_api::liteServer_accountState> from) {
    ...
    static block::AccountState
    create_account_state(ton::tl_object_ptr<ton::lite_api::liteServer_runMethodResult> &from) {
        block::AccountState res;
        res.blk = ton::create_block_id(from->id_);
        res.shard_blk = ton::create_block_id(from->shardblk_);
        res.shard_proof = std::move(from->shard_proof_);
    ...
    tonlib_api::object_ptr<tonlib_api::internal_transactionId>
    to_transaction_id(const block::AccountState::Info &info) {
    ...
    std::string to_bytes(td::Ref<vm::Cell> cell) {
        ...
        return vm::std_boc_serialize(cell,
                                     vm::BagOfCells::Mode::WithCRC32C).move_as_ok().as_slice().str();
    }

    td::Result<block::PublicKey> get_public_key(td::Slice public_key) {
        TRY_RESULT_PREFIX(address, block::PublicKey::parse(public_key),
                          TonlibError::InvalidPublicKey());
        return address;
    }

    td::Result<block::StdAddress> get_account_address(td::Slice account_address) {
        TRY_RESULT_PREFIX(address, block::StdAddress::parse(account_address),
                          TonlibError::InvalidAccountAddress());
        return address;
    }
    ...
    to_init_data(const tonlib_api::rwallet_initialAccountState &rwallet_state) {
    ...
    td::Result<ton::pchan::Config>
    to_pchan_config(const tonlib_api::pchan_initialAccountState &pchan_state) {
        ...
        TRY_RESULT_PREFIX(a_addr, get_account_address(
                pchan_state.config_->alice_address_->account_address_),
        ...
        TRY_RESULT_PREFIX(b_addr,
                          get_account_address(pchan_state.config_->bob_address_->account_address_),
    ...
    public:
        ...
        td::Result<tonlib_api::object_ptr<tonlib_api::raw_accountState>>
        to_raw_accountState() const {
            ...
            return tonlib_api::make_object<tonlib_api::raw_accountState>(std::move(code),
                                                                         std::move(data),
                                                                         raw().frozen_hash);
        }

        td::Result<tonlib_api::object_ptr<tonlib_api::raw_fullAccountState>>
        to_raw_fullAccountState() const {
            auto state = get_smc_state();
            std::string code;
            if (state.code.not_null()) {
                code = to_bytes(state.code);
            ...
            return tonlib_api::make_object<tonlib_api::raw_fullAccountState>(
                    get_balance(), std::move(code), std::move(data), to_transaction_id(raw().info),
                    to_tonlib_api(raw().block_id),
                    raw().frozen_hash, get_sync_time());
        }

        td::Result<tonlib_api::object_ptr<tonlib_api::testWallet_accountState>>
        to_testWallet_accountState() const {
        ...
        td::Result<tonlib_api::object_ptr<tonlib_api::wallet_accountState>>
        to_wallet_accountState() const {
        ...
        td::Result<tonlib_api::object_ptr<tonlib_api::wallet_v3_accountState>>
        to_wallet_v3_accountState() const {
            ...
            return tonlib_api::make_object<tonlib_api::wallet_v3_accountState>(
                    static_cast<td::uint32>(wallet_id),
                    static_cast<td::uint32>(seqno));
        }

        td::Result<tonlib_api::object_ptr<tonlib_api::wallet_highload_v1_accountState>>
        to_wallet_highload_v1_accountState()
        const {
            ...
            TRY_RESULT(seqno, wallet.get_seqno());
            TRY_RESULT(wallet_id, wallet.get_wallet_id());
            return tonlib_api::make_object<tonlib_api::wallet_highload_v1_accountState>(
                    static_cast<td::uint32>(wallet_id),
                    static_cast<td::uint32>(seqno));
        }

        td::Result<tonlib_api::object_ptr<tonlib_api::wallet_highload_v2_accountState>>
        to_wallet_highload_v2_accountState()
        const {
            ...
            TRY_RESULT(wallet_id, wallet.get_wallet_id());
            return tonlib_api::make_object<tonlib_api::wallet_highload_v2_accountState>(
                    static_cast<td::uint32>(wallet_id));
        }

        td::Result<tonlib_api::object_ptr<tonlib_api::rwallet_accountState>>
        to_rwallet_accountState() const {
            ...
            for (auto &limit : config.limits) {
                api_config->limits_.push_back(
                        tonlib_api::make_object<tonlib_api::rwallet_limit>(limit.first,
                                                                           limit.second));
            }

            return tonlib_api::make_object<tonlib_api::rwallet_accountState>(wallet_id, seqno,
                                                                             balance,
                                                                             std::move(api_config));
        }

        td::Result<tonlib_api::object_ptr<tonlib_api::pchan_accountState>>
        to_payment_channel_accountState() const {
            ...
            info.state.visit(td::overloaded(
                    [&](const ton::pchan::StateInit &state) {
                        tl_state = tonlib_api::make_object<tonlib_api::pchan_stateInit>(
                                state.signed_A, state.signed_B, state.min_A, state.min_B, state.A,
                                state.B, state.expire_at);
                    },
                    [&](const ton::pchan::StateClose &state) {
                        tl_state = tonlib_api::make_object<tonlib_api::pchan_stateClose>(
                                state.signed_A, state.signed_B, state.promise_A, state.promise_B,
                                state.A, state.B, state.expire_at);
                    },
                    [&](const ton::pchan::StatePayout &state) {
                        tl_state = tonlib_api::make_object<tonlib_api::pchan_statePayout>(state.A,
                                                                                          state.B);
                    }));
            ...
            return tonlib_api::make_object<tonlib_api::pchan_accountState>(
                    tonlib_api::make_object<tonlib_api::pchan_config>(
                            a_key.serialize(true), make_object<tonlib_api::accountAddress>(
                                    info.config.a_addr.rserialize(true)),
                            b_key.serialize(true), make_object<tonlib_api::accountAddress>(
                                    info.config.b_addr.rserialize(true)),
                            info.config.init_timeout, info.config.close_timeout,
                            info.config.channel_id),
                    std::move(tl_state), info.description);
        }

        td::Result<tonlib_api::object_ptr<tonlib_api::testGiver_accountState>>
        to_testGiver_accountState() const {
        ...
        td::Result<tonlib_api::object_ptr<tonlib_api::dns_accountState>>
        to_dns_accountState() const {
            ...
            return tonlib_api::make_object<tonlib_api::dns_accountState>(
                    static_cast<td::uint32>(wallet_id));
        }

        td::Result<tonlib_api::object_ptr<tonlib_api::AccountState>> to_accountState() const {
            auto f = [](
                    auto &&r_x) -> td::Result<tonlib_api::object_ptr<tonlib_api::AccountState>> {
        ...
        td::Result<tonlib_api::object_ptr<tonlib_api::fullAccountState>>
        to_fullAccountState() const {
            ...
            return tonlib_api::make_object<tonlib_api::fullAccountState>(
                    tonlib_api::make_object<tonlib_api::accountAddress>(
                            get_address().rserialize(true)), get_balance(),
                    to_transaction_id(raw().info), to_tonlib_api(raw().block_id), get_sync_time(),
        ...
        WalletType
        guess_type_by_init_state(tonlib_api::InitialAccountState &initial_account_state) {
            ...
            downcast_call(
                    ...
                    td::overloaded(
                            ...
                            [&](tonlib_api::rwallet_initialAccountState &rwallet) {
                                for (auto revision : ton::SmartContractCode::get_revisions(
                                        ton::SmartContractCode::RestrictedWallet)) {
                                    auto r_init_data = to_init_data(rwallet);
                                    if (r_init_data.is_error()) {
                                        continue;
                                    }
                                    auto wallet = ton::RestrictedWallet::create(
                                            r_init_data.move_as_ok(), revision);
                            ...
                            [&](tonlib_api::pchan_initialAccountState &pchan) {
                                for (auto revision : ton::SmartContractCode::get_revisions(
                                        ton::SmartContractCode::PaymentChannel)) {
        ...
        WalletType guess_type_by_public_key(td::Ed25519::PublicKey &key) {
            ...
            if (o_revision) {
                ...
                set_new_state({ton::WalletV3::get_init_code(wallet_revision_),
                               ton::WalletV3::get_init_data(key, wallet_id)});
                return wallet_type_;
            }
            o_revision = ton::HighloadWalletV2::guess_revision(address_, key,
                                                               wallet_id + address_.workchain);
            if (o_revision) {
                wallet_type_ = WalletType::HighloadWalletV2;
                wallet_revision_ = o_revision.value();
                set_new_state({ton::HighloadWalletV2::get_init_code(wallet_revision_),
                               ton::HighloadWalletV2::get_init_data(key, wallet_id +
                                                                         address_.workchain)});
                return wallet_type_;
            }
            ...
            if (o_revision) {
                ...
                wallet_revision_ = o_revision.value();
                set_new_state(
                        {ton::HighloadWallet::get_init_code(wallet_revision_),
                         ton::HighloadWallet::get_init_data(key, wallet_id)});
                return wallet_type_;
            }
            ...
            if (ton::GenericAccount::get_address(address_.workchain,
                                                 ton::TestWallet::get_init_state(key)).addr ==
                address_.addr) {
                set_new_state(
                        {ton::TestWallet::get_init_code(), ton::TestWallet::get_init_data(key)});
                wallet_type_ = WalletType::SimpleWallet;
            } else if (ton::GenericAccount::get_address(address_.workchain,
                                                        ton::Wallet::get_init_state(key)).addr ==
                       address_.addr) {
        ...
        WalletType guess_type_default(td::Ed25519::PublicKey &key) {
            ...
            set_new_state({ton::WalletV3::get_init_code(),
                           ton::WalletV3::get_init_data(key, wallet_id_)});
            wallet_type_ = WalletType::WalletV3;
            return wallet_type_;
        }
    ...
    public:
        ...
        static td::RefInt256
        compute_gas_price(td::uint64 gas_used, const block::GasLimitsPrices &cfg) {
            auto gas_price256 = td::RefInt256{true, cfg.gas_price};
            return gas_used <= cfg.flat_gas_limit
                   ? td::make_refint(cfg.flat_gas_price)
                   : td::rshift(gas_price256 * (gas_used - cfg.flat_gas_limit), 16, 1) +
                     cfg.flat_gas_price;
        }

        static vm::GasLimits
        compute_gas_limits(td::RefInt256 balance, const block::GasLimitsPrices &cfg) {
            vm::GasLimits res;
            // Compute gas limits
            ...
            if (false /*trans_type != tr_ord*/) {
                ...
                res.gas_limit = gas_bought_for(td::make_refint(0) /*msg balance remaining*/,
                                               compute_threshold(cfg), cfg);
                if (true /*!block::tlb::t_Message.is_internal(in_msg)*/) {
                    ...
                    res.gas_credit = std::min(static_cast<td::int64>(cfg.gas_credit),
                                              static_cast<td::int64>(res.gas_max));
                }
            }
            LOG(DEBUG)
            << "gas limits: max=" << res.gas_max << ", limit=" << res.gas_limit << ", credit="
            << res.gas_credit;
            return res;
        }
        ...
        struct Fee {
            ...
            auto to_tonlib_api() const {
                return tonlib_api::make_object<tonlib_api::fees>(in_fwd_fee, storage_fee, gas_fee,
                                                                 fwd_fee);
            }
        };

        td::Result<td::int64>
        calc_fwd_fees(td::Ref<vm::Cell> list, block::MsgPrices **msg_prices, bool is_masterchain) {
            td::int64 res = 0;
            ...
            while (true) {
                ...
                if (!cs.have_refs()) {
                    return td::Status::Error(
                            "action list invalid: entry found with data but no next reference");
                }
                list = cs.prefetch_ref();
                n++;
                if (n > max_actions) {
                    return td::Status::Error(
                            PSLICE() << "action list too long: more than " << max_actions
                                     << " actions");
                }
            }
            for (int i = n - 1; i >= 0; --i) {
                ...
                switch (tag) {
                    ...
                    case block::gen::OutAction::action_send_msg: {
                        ...
                        if (!tlb::unpack_exact(cs, act_rec) || (act_rec.mode & ~0xe3) ||
                            (act_rec.mode & 0xc0) == 0xc0) {
                            return td::Status::Error("estimate_fee: can't parse send_msg");
                        }
                        block::gen::MessageRelaxed::Record msg;
                        if (!tlb::type_unpack_cell(act_rec.out_msg,
                                                   block::gen::t_MessageRelaxed_Any, msg)) {
                            return td::Status::Error("estimate_fee: can't parse send_msg");
                        }

                        bool dest_is_masterchain = false;
                        if (block::gen::t_CommonMsgInfoRelaxed.get_tag(*msg.info) ==
                            block::gen::CommonMsgInfoRelaxed::int_msg_info) {
                            ...
                            if (!dest_addr->prefetch_ulong(1)) {
                                return td::Status::Error(
                                        "estimate_fee: messages with external addresses are unsupported");
                            }
                        ...
                        sstat.add_used_storage(msg.body, true,
                                               3);  // message body (the root cell itself is not counted)
                        res += msg_prices[is_masterchain || dest_is_masterchain]->compute_fwd_fees(
                                sstat.cells, sstat.bits);
                        break;
                    }
        ...
        td::Result<std::pair<Fee, std::vector<Fee>>>
        estimate_fees(bool ignore_chksig, const block::Config &cfg) {
            // gas fees
            ...
            vm::GasLimits gas_limits = compute_gas_limits(
                    td::make_refint(raw_.source->get_balance()), gas_limits_prices);
            auto res = smc.write().send_external_message(raw_.message_body,
                                                         ton::SmartContract::Args()
                                                                 .set_limits(gas_limits)
                                                                 .set_balance(
                                                                         raw_.source->get_balance())
                                                                 .set_now(
                                                                         raw_.source->get_sync_time())
                                                                 .set_ignore_chksig(ignore_chksig));
            td::int64 fwd_fee = 0;
            if (res.success) {
                LOG(DEBUG) << "output actions:\n"
                           << block::gen::OutList{output_actions_count(res.actions)}.as_string_ref(
                                   res.actions);

                TRY_RESULT_ASSIGN(fwd_fee, calc_fwd_fees(res.actions, msg_prices, is_masterchain));
            }

            auto gas_fee = res.accepted ? compute_gas_price(res.gas_used,
                                                            gas_limits_prices)->to_long() : 0;
            LOG(INFO) << storage_fee << " " << in_fwd_fee << " " << gas_fee << " " << fwd_fee << " "
                      << res.gas_used;
            ...
            for (auto &destination : raw_.destinations) {
                bool dest_is_masterchain =
                        destination && destination->get_address().workchain == ton::masterchainId;
                ...
                auto dest_storage_fee_256 =
                        destination ? block::StoragePrices::compute_storage_fees(
                                destination->get_sync_time(), storage_prices,
                                destination->raw().storage_stat,
                                destination->raw().storage_last_paid, false, is_masterchain)
                                    : td::make_refint(0);
                Fee dst_fee;
                auto dest_storage_fee = dest_storage_fee_256.is_null() ? 0
                                                                       : dest_storage_fee_256->to_long();
                if (destination &&
                    destination->get_wallet_type() != AccountState::WalletType::Empty) {
    ...
    public:
        GetTransactionHistory(ExtClientRef ext_client_ref, block::StdAddress address,
                              ton::LogicalTime lt, ton::Bits256 hash,
                              td::actor::ActorShared<> parent,
                              td::Promise<block::TransactionList::Info> promise)
                : address_(std::move(address)), lt_(std::move(lt)), hash_(std::move(hash)),
    ...
    private:
        ...
        td::Status do_with_transactions(
            ...
            TRY_RESULT_PREFIX(info, TRY_VM(do_with_transactions(std::move(transactions))),
                              TonlibError::ValidateTransactions());
        ...
        td::Result<block::TransactionList::Info>
        do_with_transactions(std::vector<ton::BlockIdExt> blkids,
            ...
            if (info.transactions.size() > static_cast<size_t>(count_)) {
                LOG(WARNING)
                << "obtained " << info.transactions.size() << " transaction, but only " << count_
                << " have been requested";
            }
            return info;
        }
        ...
        void start_up() override {
            ...
            client_.send_query(
                    ton::lite_api::liteServer_getTransactions(
                            count_, ton::create_tl_object<ton::lite_api::liteServer_accountId>(
                                    address_.workchain, address_.addr), lt_,
                            hash_),
                    [self = this](auto r_transactions) {
                        self->with_transactions(std::move(r_transactions));
                    });
        }
    ...
    public:
        RemoteRunSmcMethod(ExtClientRef ext_client_ref, int_api::RemoteRunSmcMethod query,
                           td::actor::ActorShared<> parent,
                           td::Promise<int_api::RemoteRunSmcMethod::ReturnType> &&promise)
                : query_(std::move(query)), promise_(std::move(promise)),
    ...
    private:
        ...
        td::Status do_with_run_method_result(
            ...
            TRY_RESULT_PREFIX(state,
                              TRY_VM(do_with_run_method_result(std::move(run_method_result))),
    ...
    public:
        GetRawAccountState(ExtClientRef ext_client_ref, block::StdAddress address,
                           td::optional<ton::BlockIdExt> block_id,
                           td::actor::ActorShared<> parent, td::Promise<RawAccountState> &&promise)
                : address_(std::move(address)), block_id_(std::move(block_id)),
    ...
    private:
        ...
        td::Result<RawAccountState> do_with_account_state(
            ...
            }
            {
                ...
                u |= storage_stat.public_cells = block::tlb::t_VarUInteger_7.as_uint(
                        *storage_used.public_cells);
    ...
    TonlibClient::TonlibClient(td::unique_ptr<TonlibCallback> callback) : callback_(
            std::move(callback)) {
    }
    ...
    void TonlibClient::init_ext_client() {
        if (use_callbacks_for_network_) {
            ...
            public:
                explicit Callback(td::actor::ActorShared<TonlibClient> parent,
                                  td::uint32 config_generation)
                        : parent_(std::move(parent)), config_generation_(config_generation) {
                }

                void request(td::int64 id, std::string data) override {
                    send_closure(parent_, &TonlibClient::proxy_request,
                                 (id << 16) | (config_generation_ & 0xffff),
                                 std::move(data));
                }
    ...
    void TonlibClient::update_sync_state(LastBlockSyncState state, td::uint32 config_generation) {
        ...
        switch (state.type) {
            case LastBlockSyncState::Done:
                on_update(
                        tonlib_api::make_object<tonlib_api::updateSyncState>(
                                tonlib_api::make_object<tonlib_api::syncStateDone>()));
                break;
            case LastBlockSyncState::InProgress:
                on_update(
                        tonlib_api::make_object<tonlib_api::updateSyncState>(
                                tonlib_api::make_object<tonlib_api::syncStateInProgress>(
    ...
    void TonlibClient::init_last_block(LastBlockState state) {
        ...
        public:
            ...
            void on_sync_state_changed(LastBlockSyncState sync_state) override {
                send_closure(client_, &TonlibClient::update_sync_state, std::move(sync_state),
                             config_generation_);
            }
        ...
        raw_last_block_ = td::actor::create_actor<LastBlock>(
                td::actor::ActorOptions().with_name("LastBlock").with_poll(false), get_client_ref(),
                std::move(state), config_,
    ...
    void TonlibClient::init_last_config() {
        ...
        raw_last_config_ =
                td::actor::create_actor<LastConfig>(
    ...
    void
    TonlibClient::on_result(td::uint64 id, tonlib_api::object_ptr<tonlib_api::Object> response) {
        VLOG_IF(tonlib_query, id != 0)
        << "Tonlib answer query " << td::tag("id", id) << " " << to_string(response);
    ...
    void TonlibClient::make_any_request(tonlib_api::Function &function, QueryContext query_context,
        ...
        downcast_call(function, [&](auto &request) {
            this->make_request(request, promise.wrap([](auto x) { return x; }));
        });
    }

    void
    TonlibClient::request(td::uint64 id, tonlib_api::object_ptr<tonlib_api::Function> function) {
        VLOG(tonlib_query)
        << "Tonlib got query " << td::tag("id", id) << " " << to_string(function);
        if (function == nullptr) {
            LOG(ERROR) << "Receive empty static request";
            return on_result(id,
                             tonlib_api::make_object<tonlib_api::error>(400, "Request is empty"));
        }
        ...
        if (state_ == State::Closed) {
            return on_result(id,
                             tonlib_api::make_object<tonlib_api::error>(400, "tonlib is closed"));
        }
        if (state_ == State::Uninited) {
            if (!is_uninited_request(function->get_id())) {
                return on_result(id, tonlib_api::make_object<tonlib_api::error>(400,
                                                                                "library is not inited"));
            }
        }
    ...
    tonlib_api::object_ptr<tonlib_api::Object>
    TonlibClient::do_static_request(const tonlib_api::runTests &request) {
    ...
    td::Result<block::StdAddress>
    get_account_address(const tonlib_api::raw_initialAccountState &raw_state,
                        td::int32 revision, ton::WorkchainId workchain_id) {
        TRY_RESULT_PREFIX(code, vm::std_boc_deserialize(raw_state.code_),
                          TonlibError::InvalidBagOfCells("raw_state.code"));
        TRY_RESULT_PREFIX(data, vm::std_boc_deserialize(raw_state.data_),
                          TonlibError::InvalidBagOfCells("raw_state.data"));
        return ton::GenericAccount::get_address(workchain_id,
                                                ton::GenericAccount::get_init_state(std::move(code),
                                                                                    std::move(
                                                                                            data)));
    }

    td::Result<block::StdAddress>
    get_account_address(const tonlib_api::testGiver_initialAccountState &test_wallet_state,
                        td::int32 revision, ton::WorkchainId workchain_id) {
        return ton::TestGiver::address();
    }

    td::Result<block::StdAddress>
    get_account_address(const tonlib_api::testWallet_initialAccountState &test_wallet_state,
                        td::int32 revision, ton::WorkchainId workchain_id) {
        ...
        return ton::GenericAccount::get_address(workchain_id,
                                                ton::TestWallet::get_init_state(key, revision));
    }

    td::Result<block::StdAddress>
    get_account_address(const tonlib_api::wallet_initialAccountState &wallet_state,
                        td::int32 revision, ton::WorkchainId workchain_id) {
        TRY_RESULT(key_bytes, get_public_key(wallet_state.public_key_));
        auto key = td::Ed25519::PublicKey(td::SecureString(key_bytes.key));
        return ton::GenericAccount::get_address(workchain_id,
                                                ton::Wallet::get_init_state(key, revision));
    }

    td::Result<block::StdAddress>
    get_account_address(const tonlib_api::wallet_v3_initialAccountState &test_wallet_state,
                        td::int32 revision, ton::WorkchainId workchain_id) {
        TRY_RESULT(key_bytes, get_public_key(test_wallet_state.public_key_));
        auto key = td::Ed25519::PublicKey(td::SecureString(key_bytes.key));
        return ton::GenericAccount::get_address(
                workchain_id,
                ton::WalletV3::get_init_state(key,
                                              static_cast<td::uint32>(test_wallet_state.wallet_id_),
                                              revision));
    }

    td::Result<block::StdAddress> get_account_address(
            const tonlib_api::wallet_highload_v1_initialAccountState &test_wallet_state,
            td::int32 revision,
        ...
        return ton::GenericAccount::get_address(
                workchain_id,
                ton::HighloadWallet::get_init_state(key,
                                                    static_cast<td::uint32>(test_wallet_state.wallet_id_),
                                                    revision));
    }

    td::Result<block::StdAddress> get_account_address(
            const tonlib_api::wallet_highload_v2_initialAccountState &test_wallet_state,
            td::int32 revision,
            ton::WorkchainId workchain_id) {
        TRY_RESULT(key_bytes, get_public_key(test_wallet_state.public_key_));
        auto key = td::Ed25519::PublicKey(td::SecureString(key_bytes.key));
        return ton::GenericAccount::get_address(
                workchain_id,
                ton::HighloadWalletV2::get_init_state(key,
                                                      static_cast<td::uint32>(test_wallet_state.wallet_id_),
                                                      revision));
    }

    td::Result<block::StdAddress>
    get_account_address(const tonlib_api::dns_initialAccountState &dns_state,
                        td::int32 revision, ton::WorkchainId workchain_id) {
    ...
    td::Result<block::StdAddress>
    get_account_address(const tonlib_api::pchan_initialAccountState &pchan_state,
                        td::int32 revision, ton::WorkchainId workchain_id) {
    ...
    td::Result<block::StdAddress>
    get_account_address(const tonlib_api::rwallet_initialAccountState &rwallet_state,
                        td::int32 revision, ton::WorkchainId workchain_id) {
    ...
    static td::optional<ton::SmartContractCode::Type>
    get_wallet_type(tonlib_api::InitialAccountState &state) {
    ...
    tonlib_api::object_ptr<tonlib_api::Object> TonlibClient::do_static_request(
        ...
        if (o_type) {
            auto status = ton::SmartContractCode::validate_revision(o_type.value(),
                                                                    request.revision_);
            if (status.is_error()) {
        ...
        auto r_account_address = downcast_call2<td::Result<block::StdAddress>>(
                *request.initial_account_state_,
                [&request](auto &&state) {
                    return get_account_address(state, request.revision_, request.workchain_id_);
                });
        ...
        return tonlib_api::make_object<tonlib_api::accountAddress>(
                r_account_address.ok().rserialize(true));
    }
    ...
    td::Status TonlibClient::do_request(tonlib_api::guessAccountRevision &request,
        ...
        for (auto &initial_account_state : states) {
            ...
            TRY_STATUS(downcast_call2<td::Status>(
                    *initial_account_state,
                    [&revisions, &targets, &workchains, &type](const auto &state) {
                        for (auto workchain : workchains) {
                            for (auto revision : revisions) {
                                TRY_RESULT(address,
                                           get_account_address(state, revision, workchain));
                                Target target;
                                target.can_be_empty =
                                        type != ton::SmartContractCode::Type::RestrictedWallet;
    ...
    td::Status TonlibClient::do_request(tonlib_api::guessAccount &request,
        ...
        for (Source &source : sources) {
            ...
            TRY_STATUS(downcast_call2<td::Status>(
                    *source.init_state,
                    [&revisions, &targets, &workchains, &type, &key_bytes](const auto &state) {
                        for (auto workchain : workchains) {
                            for (auto revision : revisions) {
                                TRY_RESULT(address,
                                           get_account_address(state, revision, workchain));
                                ...
                                target.can_be_uninited =
                                        type == ton::SmartContractCode::Type::WalletV3 &&
                                        revision == 2 && workchain == ton::basechainId;
                                target.can_be_empty =
                                        type != ton::SmartContractCode::Type::RestrictedWallet ||
                                        target.can_be_uninited;
                                target.address = address;
                                target.public_key = td::Ed25519::PublicKey(
                                        td::SecureString(key_bytes.key));
                                targets.push_back(std::move(target));
                            }
                        }
                        return td::Status::OK();
    ...
    td::Status TonlibClient::guess_revisions(std::vector<Target> targets,
        ...
        public:
            GuessRevisions(td::actor::ActorShared<TonlibClient> client,
                           td::optional<ton::BlockIdExt> block_id,
                           std::vector<Target> targets,
                           td::Promise<std::vector<td::unique_ptr<AccountState>>> promise)
                    : TonlibQueryActor(std::move(client)), block_id_(std::move(block_id)),
                      targets_(std::move(targets)), promise_(std::move(promise)) {
            }
        ...
        private:
            ...
            void start_up() override {
                left_ += targets_.size();
                for (auto &p : targets_) {
                    send_query(int_api::GetAccountState{p.address, block_id_.copy(),
                                                        std::move(p.public_key)},
                               promise_send_closure(td::actor::actor_id(this),
                                                    &GuessRevisions::on_account_state,
                                                    std::move(p)));
                }
            ...
            void on_account_state(Target target, td::Result<td::unique_ptr<AccountState>> r_state) {
                ...
                if (state->get_wallet_type() == AccountState::WalletType::Empty &&
                    !target.can_be_empty) {
                    return;
                }
                    ...
                    std::sort(res.begin(), res.end(), [](auto &x, auto &y) {
                        auto key = [](const td::unique_ptr<AccountState> &state) {
                            return std::make_tuple(
                                    state->get_wallet_type() != AccountState::WalletType::Empty,
                                    state->get_wallet_type(), state->get_balance(),
                                    state->get_wallet_revision());
                        };
                        return key(x) > key(y);
                    });
        ...
        actors_[actor_id] = td::actor::create_actor<GuessRevisions>(
                "GuessRevisions", actor_shared(this, actor_id), query_context_.block_id.copy(),
                std::move(targets),
    ...
    tonlib_api::object_ptr<tonlib_api::Object> TonlibClient::do_static_request(
        ...
        if (request.account_address_->addr_.size() != 32) {
            return status_to_tonlib_api(
                    TonlibError::InvalidField("account_address.addr", "must be 32 bytes long"));
        }
    ...
    tonlib_api::object_ptr<tonlib_api::Object>
    TonlibClient::do_static_request(tonlib_api::getBip39Hints &request) {
        return tonlib_api::make_object<tonlib_api::bip39Hints>(
                td::transform(Mnemonic::word_hints(td::trim(td::to_lower_inplace(request.prefix_))),
                              [](auto &x) { return x; }));
    }
    ...
    td::Status TonlibClient::do_request(const tonlib_api::init &request,
        ...
        auto r_kv = downcast_call2<td::Result<td::unique_ptr<KeyValue>>>(
                *request.options_->keystore_type_,
                td::overloaded(
                        [](tonlib_api::keyStoreTypeDirectory &directory) {
                            return KeyValue::create_dir(directory.directory_);
                        },
...
{"workchain":-1,"shard":-9223372036854775808,"seqno":870721,"root_hash":"jYKhSQ1xeSPprzgjqiUOnAWwc2yqs7nCVAU21k922s4=","file_hash":"kHidF02CZpaz2ia9jtXUJLp0AiWMWwfzprTUIsddHSo="}
  ...
  "validator": {
   ...
   "hardforks": [
    ...
    td::Result<TonlibClient::FullConfig>
    TonlibClient::validate_config(tonlib_api::object_ptr<tonlib_api::config> config) {
        if (!config) {
            return TonlibError::EmptyField("config");
        }
        ...
        if (config->blockchain_name_.empty()) {
            ...
            o_master_config = get_default_master_config().by_root_hash(
                    new_config.zero_state_id.root_hash);
        } else {
        ...
        if (o_master_config) {
            ...
            if (!name.empty() && !new_config.name.empty() && new_config.name != name &&
                name == "mainnet") {
                return TonlibError::InvalidConfig(
                        PSLICE() << "Invalid blockchain_id: expected '" << name << "'");
            }
        }
        ...
        auto zero_state = ton::ZeroStateIdExt(new_config.zero_state_id.id.workchain,
                                              new_config.zero_state_id.root_hash,
                                              new_config.zero_state_id.file_hash);
        if (config->ignore_cache_ || r_state.is_error()) {
            LOG_IF(WARNING, !config->ignore_cache_)
            << "Unknown LastBlockState: " << r_state.error();
            ...
            if (state.vert_seqno > vert_seqno) {
                LOG(ERROR)
                << "Stored vert_seqno is bigger than one in config: " << state.vert_seqno << " vs "
                << vert_seqno;
        ...
        if (o_master_config) {
            ...
            if (master_config.init_block_id.is_valid() &&
                ...
                LOG(INFO)
                << "Use init block from MASTER config: " << master_config.init_block_id.to_str();
            }
            if (!master_config.name.empty()) {
                if (new_config.name != master_config.name) {
                    LOG(INFO) << "Use blockchain name from MASTER config: '" << master_config.name
                              << "' (was '" << new_config.name
                              << "')";
    ...
    struct ToRawTransactions {
        explicit ToRawTransactions(td::optional<td::Ed25519::PrivateKey> private_key)
                : private_key_(std::move(private_key)) {
        }

        td::optional<td::Ed25519::PrivateKey> private_key_;

        td::Result<tonlib_api::object_ptr<tonlib_api::raw_message>>
        to_raw_message_or_throw(td::Ref<vm::Cell> cell) {
            ...
            auto get_data = [body = std::move(body), body_cell, this](td::Slice salt) mutable {
                ...
                if (body->size() >= 32 && static_cast<td::uint32>(body->prefetch_long(32)) <= 1) {
                    ...
                    LOG_IF(WARNING, r_body_message.is_error())
                    << "Failed to parse a message: " << r_body_message.error();

                    if (r_body_message.is_ok()) {
                        if (type == 0) {
                            data = tonlib_api::make_object<tonlib_api::msg_dataText>(
                                    r_body_message.move_as_ok());
                        } else {
                            ...
                            auto r_decrypted_message = [&]() -> td::Result<std::string> {
                                ...
                                TRY_RESULT(decrypted,
                                           SimpleEncryptionV2::decrypt_data(encrypted_message,
                                                                            private_key_.value(),
                                                                            salt));
                                return decrypted.data.as_slice().str();
                            }();
                            if (r_decrypted_message.is_ok()) {
                                data = tonlib_api::make_object<tonlib_api::msg_dataDecryptedText>(
                                        r_decrypted_message.move_as_ok());
                            } else {
                                data = tonlib_api::make_object<tonlib_api::msg_dataEncryptedText>(
                                        encrypted_message);
                            }
                        }
                    }
                ...
                if (!data) {
                    data = tonlib_api::make_object<tonlib_api::msg_dataRaw>(
                            to_bytes(std::move(body_cell)), "");
                }
                return data;
            };
            ...
            switch (tag) {
                case block::gen::CommonMsgInfo::int_msg_info: {
                    ...
                    return tonlib_api::make_object<tonlib_api::raw_message>(
                            ...
                            tonlib_api::make_object<tonlib_api::accountAddress>(std::move(dest)),
                            balance, fwd_fee, ihr_fee, created_lt,
                ...
                case block::gen::CommonMsgInfo::ext_out_msg_info: {
                    ...
                    if (!tlb::csr_unpack(message.info, msg_info)) {
                        return td::Status::Error(
                                "Failed to unpack CommonMsgInfo::ext_out_msg_info");
                    }
                    TRY_RESULT(src, to_std_address(msg_info.src));
                    return tonlib_api::make_object<tonlib_api::raw_message>(
                            tonlib_api::make_object<tonlib_api::accountAddress>(src),
                            tonlib_api::make_object<tonlib_api::accountAddress>(), 0, 0, 0, 0,
                            std::move(body_hash), get_data(src));
                }
            }
        ...
        td::Result<tonlib_api::object_ptr<tonlib_api::raw_message>>
        to_raw_message(td::Ref<vm::Cell> cell) {
        ...
        td::Result<tonlib_api::object_ptr<tonlib_api::raw_transaction>> to_raw_transaction_or_throw(
            ...
            if (info.transaction.not_null()) {
                ...
                if (!block::tlb::t_TransactionDescr.get_storage_fees(trans.description,
                                                                     storage_fees)) {
        ...
        td::Result<tonlib_api::object_ptr<tonlib_api::raw_transaction>>
        to_raw_transaction(block::Transaction::Info &&info) {
...
// Raw
    ...
    td::Status TonlibClient::do_request(const tonlib_api::raw_createAndSendMessage &request,
        ...
        if (!request.initial_account_state_.empty()) {
            TRY_RESULT_PREFIX(new_init_state,
                              vm::std_boc_deserialize(request.initial_account_state_),
                              TonlibError::InvalidBagOfCells("initial_account_state"));
            init_state = std::move(new_init_state);
        }
        TRY_RESULT_PREFIX(data, vm::std_boc_deserialize(request.data_),
                          TonlibError::InvalidBagOfCells("data"));
        TRY_RESULT(account_address, get_account_address(request.destination_->account_address_));
        auto message = ton::GenericAccount::create_ext_message(account_address,
                                                               std::move(init_state),
                                                               std::move(data));
    ...
    td::Status TonlibClient::do_request(tonlib_api::raw_getAccountState &request,
        ...
        TRY_RESULT(account_address,
                   get_account_address(request.account_address_->account_address_));
        make_request(
                int_api::GetAccountState{std::move(account_address), query_context_.block_id.copy(),
                                         {}},
    ...
    td::Result<KeyStorage::InputKey> from_tonlib(tonlib_api::inputKeyRegular &input_key) {
        ...
        return KeyStorage::InputKey{
                {td::SecureString(key_bytes.key), std::move(input_key.key_->secret_)},
    ...
    td::Result<KeyStorage::InputKey> from_tonlib(tonlib_api::InputKey &input_key) {
        return downcast_call2<td::Result<KeyStorage::InputKey>>(
                input_key, td::overloaded([&](tonlib_api::inputKeyRegular &input_key) {
                                              return from_tonlib(input_key);
                                          },
    ...
    td::Status TonlibClient::do_request(tonlib_api::raw_getTransactions &request,
        ...
        actors_[actor_id] = td::actor::create_actor<GetTransactionHistory>(
                ...
                promise.wrap([private_key = std::move(private_key)](auto &&x) mutable {
                    return ToRawTransactions(std::move(private_key)).to_raw_transactions(
                            std::move(x));
                }));
        return td::Status::OK();
    }
        ...
        return downcast_call2<R>(
                ...
                td::overloaded(
                        ...
                        [&](tonlib_api::dns_entryDataNextResolver &next_resolver) -> R {
                            ...
                            TRY_RESULT(resolver, get_account_address(
                                    next_resolver.resolver_->account_address_));
                        ...
                        [&](tonlib_api::dns_entryDataSmcAddress &smc_address) -> R {
                            ...
                            TRY_RESULT(address, get_account_address(
                                    smc_address.smc_address_->account_address_));
                        ...
                        [&](tonlib_api::dns_entryDataAdnlAddress &adnl_address) -> R {
                            ...
                            TRY_RESULT(address,
                                       get_adnl_address(adnl_address.adnl_address_->adnl_address_));
                            return ton::ManualDns::EntryData::adnl_address(std::move(address));
                        },
                        [&](tonlib_api::dns_entryDataText &text) -> R {
                            return ton::ManualDns::EntryData::text(text.text_);
                        }));
    }
    ...
    public:
        GenericCreateSendGrams(td::actor::ActorShared<TonlibClient> client,
                               tonlib_api::createQuery query,
                               td::optional<ton::BlockIdExt> block_id,
                               td::Promise<td::unique_ptr<Query>> &&promise)
                : TonlibQueryActor(std::move(client)), query_(std::move(query)),
                  promise_(std::move(promise)), block_id_(std::move(block_id)) {
        }
    ...
    private:
            ...
            auto status = downcast_call2<td::Status>(
                    *message.data_, td::overloaded(
                            [&](tonlib_api::msg_dataRaw &text) {
                                ...
                                TRY_RESULT(init_state,
                                           vm::std_boc_deserialize(text.init_state_, true));
            ...
            return downcast_call2<R>(action,
                                     td::overloaded(
                                             ...
                                             [&](tonlib_api::dns_actionSet &set) -> R {
                                                 ...
                                                 TRY_RESULT(category,
                                                            td::narrow_cast_safe<td::int16>(
                                                                    set.entry_->category_));
                                                 TRY_RESULT(entry_data,
                                                            to_dns_entry_data(*set.entry_->entry_));
                                                 TRY_RESULT(data_cell, entry_data.as_cell());
                                                 return ton::ManualDns::Action{set.entry_->name_,
                                                                               category, std::move(
                                                                 data_cell)};
                                             }));
        }
                                              ...
                                              td::overloaded(
                                                      ...
                                                      [&](tonlib_api::actionDns &cell) {
                                                          for (auto &from_action : cell.actions_) {
                                                              ...
                                                              TRY_RESULT(action, to_dns_action(
                                                                      *from_action));
                                                              dns_actions_.push_back(
                                                                      std::move(action));
                                                          }
                                                          return td::Status::OK();
                                                      }));
        ...
        td::Status do_start_up() {
            ...
            if (has_private_key_) {
                ...
                send_query(int_api::GetPrivateKey{std::move(input_key)},
                           promise_send_closure(actor_id(this),
                                                &GenericCreateSendGrams::on_private_key));
            }
            TRY_STATUS(parse_action(*query_.action_));

            send_query(int_api::GetAccountState{source_address, block_id_.copy(), {}},
                       promise_send_closure(actor_id(this),
                                            &GenericCreateSendGrams::on_source_state));
            ...
            for (size_t i = 0; i < actions_.size(); i++) {
                send_query(int_api::GetAccountState{actions_[i].destination, block_id_.copy(), {}},
                           promise_send_closure(actor_id(this),
                                                &GenericCreateSendGrams::on_destination_state, i));
            }
        ...
        td::Status do_on_source_state(td::Result<td::unique_ptr<AccountState>> r_state) {
            ...
            if (source_->get_wallet_type() == AccountState::Empty &&
                query_.initial_account_state_) {
        ...
        td::Status
        do_on_destination_state(size_t i, td::Result<td::unique_ptr<AccountState>> r_state) {
            TRY_RESULT(state, std::move(r_state));
            ...
            if (destination.get_wallet_type() == AccountState::Empty &&
                destination.get_address().bounceable) {
        ...
        td::Status do_dns_loop() {
            ...
            if (dns_actions_.empty()) {
                ...
                TRY_RESULT(message_body,
                           dns->create_update_query(private_key_.value(), dns_actions_,
                                                    valid_until));
                raw.message_body = std::move(message_body);
            }
            raw.new_state = source_->get_new_state();
            raw.message = ton::GenericAccount::create_ext_message(source_->get_address(),
        ...
        td::Status
        do_pchan_loop(td::Ref<ton::PaymentChannel> pchan, tonlib_api::actionPchan &action) {
            if (!action.action_) {
                return TonlibError::EmptyField("action");
            }
            ...
            bool is_alice = false;
            bool is_bob = false;
            if (info.config.a_key ==
                private_key_.value().get_public_key().move_as_ok().as_octet_string()) {
                LOG(ERROR) << "Alice key";
                is_alice = true;
            } else if (info.config.b_key ==
                       private_key_.value().get_public_key().move_as_ok().as_octet_string()) {
                LOG(ERROR) << "Bob key";
                is_bob = true;
            }
                    ...
                    *action.action_,
                    td::overloaded(
                            ...
                            [&](tonlib_api::pchan_actionClose &close) {
                                ...
                                if (is_alice &&
                                    !sbuilder.check_signature(close.promise_->signature_,
                                                              td::Ed25519::PublicKey(
                                                                      info.config.b_key.copy()))) {
                                    return TonlibError::InvalidSignature();
                                }
                                if (is_bob && !sbuilder.check_signature(close.promise_->signature_,
                                                                        td::Ed25519::PublicKey(
                                                                                info.config.a_key.copy()))) {
                                    return TonlibError::InvalidSignature();
                                }
            ...
            return downcast_call2<td::Status>(
                    *query_.action_,
                    td::overloaded([&](tonlib_api::actionNoop &cell) { return td::Status::OK(); },
                                   ...
                                   [&](tonlib_api::actionPchan &cell) {
                                       return do_pchan_loop(pchan, cell);
                                   }));
        }

        td::Status do_rwallet_action(td::Ref<ton::RestrictedWallet> rwallet,
                                     tonlib_api::actionRwallet &action) {
            if (!action.action_) {
                return TonlibError::EmptyField("action");
            }
            ...
            TRY_RESULT_PREFIX(message_body,
                              rwallet->get_init_message(private_key_.value(), valid_until, config),
            ...
            return downcast_call2<td::Status>(
                    *query_.action_,
                    td::overloaded([&](auto &cell) { return td::Status::Error("UNREACHABLE"); },
                                   [&](tonlib_api::actionRwallet &cell) {
                                       return do_rwallet_action(rwallet, cell);
                                   }));
        }
        ...
        td::Status do_loop() {
            ...
            if (source_->get_wallet_type() == AccountState::RestrictedWallet) {
                ...
                if (r_unlocked_balance.is_ok() &&
                    amount > static_cast<td::int64>(r_unlocked_balance.ok())) {
                    return TonlibError::NotEnoughFunds();
                }
            }
            ...
            for (auto &action : actions_) {
                ...
                if (action.body.not_null()) {
                    ...
                    if (!o_public_key) {
                        ...
                        auto r_public_key = ton::GenericAccount::get_public_key(
                                destination->get_smc_state());
                        if (r_public_key.is_ok()) {
                            o_public_key = r_public_key.move_as_ok();
                        }
                    }
                    ...
                    TRY_RESULT_PREFIX(encrypted_message,
                                      SimpleEncryptionV2::encrypt_data(action.message,
                                                                       o_public_key.unwrap(),
                                                                       private_key_.value(),
            ...
            auto with_wallet = [&](auto &&wallet) {
                ...
                raw.valid_until = valid_until;
                TRY_RESULT(message_body,
                           wallet.make_a_gift_message(private_key_.unwrap(), valid_until, gifts));
                raw.message_body = std::move(message_body);
                raw.new_state = source_->get_new_state();
                raw.message = ton::GenericAccount::create_ext_message(source_->get_address(),
                                                                      raw.new_state,
    ...
    td::Result<tonlib_api::object_ptr<tonlib_api::query_info>>
    TonlibClient::get_query_info(td::int64 id) {
    ...
    td::Status TonlibClient::do_request(tonlib_api::msg_decrypt &request,
        ...
        make_request(
                ...
                promise.wrap([elements = std::move(request.data_)](auto key) mutable {
                    ...
                    auto new_elements = td::transform(std::move(elements->elements_),
                                                      [&private_key](auto msg) -> ReturnType {
                                                          ...
                                                          auto r_account_address = get_account_address(
                                                                  msg->source_->account_address_);
                                                          if (r_account_address.is_error()) {
                                                              return res;
                                                          }
                                                          return downcast_call2<ReturnType>(
                                                                  *res->data_,
                                                                  td::overloaded(
                                                                          ...
                                                                          [&res, &private_key, &msg](
                                                                                  tonlib_api::msg_dataEncryptedText &encrypted) -> ReturnType {
                                                                              auto r_decrypted =
                                                                                      SimpleEncryptionV2::decrypt_data(
                                                                                              encrypted.text_,
                                                                                              private_key,
                                                                                              msg->source_->account_address_);
                                                                              ...
                                                                              return tonlib_api::make_object<tonlib_api::msg_dataDecrypted>(
                                                                                      ...
                                                                                      tonlib_api::make_object<tonlib_api::msg_dataDecryptedText>(
                                                                                              decrypted.data.as_slice().str()));
                                                                          }));
                                                      });
                    return tonlib_api::make_object<tonlib_api::msg_dataDecryptedArray>(
    ...
    void TonlibClient::query_estimate_fees(td::int64 id, bool ignore_chksig,
                                           td::Result<LastConfigState> r_state,
        ...
        TRY_RESULT_PROMISE_PREFIX(promise, fees,
                                  TRY_VM(it->second->estimate_fees(ignore_chksig, *state.config)),
                                  TonlibError::Internal());
        promise.set_value(tonlib_api::make_object<tonlib_api::query_fees>(
                fees.first.to_tonlib_api(),
                td::transform(fees.second, [](auto &x) { return x.to_tonlib_api(); })));
    }
    ...
    td::Result<tonlib_api::object_ptr<tonlib_api::smc_info>>
    TonlibClient::get_smc_info(td::int64 id) {
    ...
    auto to_tonlib_api(
            const vm::StackEntry &entry) -> tonlib_api::object_ptr<tonlib_api::tvm_StackEntry> {
        switch (entry.type()) {
            case vm::StackEntry::Type::t_int:
                return tonlib_api::make_object<tonlib_api::tvm_stackEntryNumber>(
                        tonlib_api::make_object<tonlib_api::tvm_numberDecimal>(
                                dec_string(entry.as_int())));
            case vm::StackEntry::Type::t_slice:
                return tonlib_api::make_object<tonlib_api::tvm_stackEntryCell>(
                        tonlib_api::make_object<tonlib_api::tvm_cell>(
                                to_bytes(vm::CellBuilder().append_cellslice(
                                        entry.as_slice()).finalize())));
        ...
        return downcast_call2<td::Result<vm::StackEntry>>(
                ...
                td::overloaded(
                        [&](tonlib_api::tvm_stackEntryUnsupported &cell) {
                            return td::Status::Error("Unsuppored stack entry");
                        },
    ...
    td::Status TonlibClient::do_request(const tonlib_api::smc_runGetMethod &request,
        ...
        downcast_call(*request.method_,
                      td::overloaded([&](tonlib_api::smc_methodIdNumber &number) {
                                         args.set_method_id(number.number_);
                                     },
                                     [&](tonlib_api::smc_methodIdName &name) {
                                         args.set_method_id(name.name_);
                                     }));
        ...
        promise.set_value(tonlib_api::make_object<tonlib_api::smc_runResult>(res.gas_used,
                                                                             std::move(res_stack),
                                                                             res.code));
        return td::Status::OK();
    }
        ...
        entry_data.data.visit(td::overloaded(
                ...
                [&](const ton::ManualDns::EntryDataNextResolver &resolver) {
                    res = tonlib_api::make_object<tonlib_api::dns_entryDataNextResolver>(
                            tonlib_api::make_object<tonlib_api::accountAddress>(
                                    resolver.resolver.rserialize(true)));
                },
    ...
    void TonlibClient::finish_dns_resolve(std::string name, td::int32 category, td::int32 ttl,
        ...
        if (entries.size() == 1 && entries[0].category == -1 && entries[0].name != name &&
            ttl > 0 &&
            ...
            if (name[dot_position] != '.') {
                TRY_STATUS_PROMISE(promise, td::Status::Error(
                        "next resolver error: domain split not at a component boundary "));
            }
            if (suffix != got_name) {
                TRY_STATUS_PROMISE(promise,
                                   TonlibError::Internal("domain is not a suffix of the query"));
            }

            auto address = entries[0].data.data.get<ton::ManualDns::EntryDataNextResolver>().resolver;
            return do_dns_request(prefix, category, ttl - 1, std::move(block_id), address,
                                  std::move(promise));
        }
        ...
        for (auto &entry : entries) {
            ...
            api_entries.push_back(
                    tonlib_api::make_object<tonlib_api::dns_entry>(entry.name, entry.category,
                                                                   std::move(entry_data)));
        }
        promise.set_value(
                tonlib_api::make_object<tonlib_api::dns_resolved>(std::move(api_entries)));
    }
    ...
    void TonlibClient::do_dns_request(std::string name, td::int32 category, td::int32 ttl,
        ...
        td::Promise<DnsFinishData> new_promise =
                promise.send_closure(actor_id(this), &TonlibClient::finish_dns_resolve, name,
        ...
        if (0) {
            make_request(int_api::GetAccountState{address, std::move(block_id_copy), {}},
                         new_promise.wrap([](auto &&account_state) {
                             return DnsFinishData{account_state->get_block_id(),
                                                  account_state->get_smc_state()};
                         }));
    ...
    td::Status TonlibClient::do_request(const tonlib_api::dns_resolve &request,
        ...
        if (!request.account_address_) {
            make_request(int_api::GetDnsResolver{},
                         promise.send_closure(actor_id(this), &TonlibClient::do_dns_request,
                                              request.name_, request.category_,
                                              request.ttl_, std::move(block_id)));
            return td::Status::OK();
        }
        TRY_RESULT(account_address,
                   get_account_address(request.account_address_->account_address_));
        do_dns_request(request.name_, request.category_, request.ttl_, std::move(block_id),
                       account_address,
                       std::move(promise));
        return td::Status::OK();
    }
    ...
    td::Status TonlibClient::do_request(tonlib_api::pchan_packPromise &request,
        ...
        promise.set_value(tonlib_api::make_object<tonlib_api::data>(
                td::SecureString(to_bytes(ton::pchan::SignedPromiseBuilder()
                                                  ...
                                                  .promise_B(request.promise_->promise_B_)
                                                  .channel_id(request.promise_->channel_id_)
                                                  .signature(td::SecureString(
                                                          request.promise_->signature_))
                                                  .finalize()))));
        return td::Status::OK();
    }
    ...
    td::Status TonlibClient::do_request(tonlib_api::pchan_unpackPromise &request,
                                        td::Promise<object_ptr<tonlib_api::pchan_promise>> &&promise) {
        TRY_RESULT_PREFIX(body, vm::std_boc_deserialize(request.data_),
                          TonlibError::InvalidBagOfCells("data"));
        ...
        promise.set_value(tonlib_api::make_object<tonlib_api::pchan_promise>(
                spromise.o_signature.value().as_slice().str(), spromise.promise.promise_A,
                spromise.promise.promise_B,
    ...
    td::Status TonlibClient::do_request(tonlib_api::sync &request,
        ...
        client_.with_last_block(
                std::move(promise).wrap(
                        [](auto last_block) -> td::Result<tonlib_api::object_ptr<tonlib_api::ton_blockIdExt>> {
    ...
    td::Status TonlibClient::do_request(const tonlib_api::createNewKey &request,
        ...
        TRY_RESULT_PREFIX(
                key,
                key_storage_.create_new_key(std::move(request.local_password_),
        ...
        promise.set_value(tonlib_api::make_object<tonlib_api::key>(key_bytes.serialize(true),
                                                                   std::move(key.secret)));
        return td::Status::OK();
    }
    ...
    td::Status TonlibClient::do_request(const tonlib_api::exportKey &request,
        ...
        promise.set_value(tonlib_api::make_object<tonlib_api::exportedKey>(
                std::move(exported_key.mnemonic_words)));
        return td::Status::OK();
    }
    ...
    td::Status TonlibClient::do_request(const tonlib_api::importKey &request,
        ...
        TRY_RESULT(key, key_storage_.import_key(std::move(request.local_password_),
                                                std::move(request.mnemonic_password_),
                                                KeyStorage::ExportedKey{std::move(
                                                        request.exported_key_->word_list_)}));
        TRY_RESULT(key_bytes, public_key_from_bytes(key.public_key.as_slice()));
        promise.set_value(tonlib_api::make_object<tonlib_api::key>(key_bytes.serialize(true),
                                                                   std::move(key.secret)));
        return td::Status::OK();
    ...
    td::Status TonlibClient::do_request(const tonlib_api::exportPemKey &request,
        ...
        TRY_RESULT(input_key, from_tonlib(*request.input_key_));
        TRY_RESULT(exported_pem_key, key_storage_.export_pem_key(std::move(input_key),
                                                                 std::move(request.key_password_)));
        promise.set_value(tonlib_api::make_object<tonlib_api::exportedPemKey>(
                std::move(exported_pem_key.pem)));
        return td::Status::OK();
    }
    ...
    td::Status TonlibClient::do_request(const tonlib_api::importPemKey &request,
        ...
        TRY_RESULT(key, key_storage_.import_pem_key(std::move(request.local_password_),
                                                    std::move(request.key_password_),
                                                    KeyStorage::ExportedPemKey{std::move(
                                                            request.exported_key_->pem_)}));
        TRY_RESULT(key_bytes, public_key_from_bytes(key.public_key.as_slice()));
        promise.set_value(tonlib_api::make_object<tonlib_api::key>(key_bytes.serialize(true),
                                                                   std::move(key.secret)));
        return td::Status::OK();
    ...
    td::Status TonlibClient::do_request(const tonlib_api::exportEncryptedKey &request,
        ...
        TRY_RESULT(input_key, from_tonlib(*request.input_key_));
        TRY_RESULT(exported_key,
                   key_storage_.export_encrypted_key(std::move(input_key), request.key_password_));
        promise.set_value(tonlib_api::make_object<tonlib_api::exportedEncryptedKey>(
                std::move(exported_key.data)));
        return td::Status::OK();
    }
    ...
    td::Status TonlibClient::do_request(const tonlib_api::importEncryptedKey &request,
        ...
        TRY_RESULT(key, key_storage_.import_encrypted_key(
                ...
                KeyStorage::ExportedEncryptedKey{
                        std::move(request.exported_encrypted_key_->data_)}));
        TRY_RESULT(key_bytes, public_key_from_bytes(key.public_key.as_slice()));
        promise.set_value(tonlib_api::make_object<tonlib_api::key>(key_bytes.serialize(true),
                                                                   std::move(key.secret)));
        return td::Status::OK();
    ...
    td::Status TonlibClient::do_request(const tonlib_api::exportUnencryptedKey &request,
        ...
        promise.set_value(tonlib_api::make_object<tonlib_api::exportedUnencryptedKey>(
                std::move(exported_key.data)));
        return td::Status::OK();
    }
    ...
    td::Status TonlibClient::do_request(const tonlib_api::importUnencryptedKey &request,
        ...
        TRY_RESULT(key, key_storage_.import_unencrypted_key(
                ...
                KeyStorage::ExportedUnencryptedKey{
                        std::move(request.exported_unencrypted_key_->data_)}));
        TRY_RESULT(key_bytes, public_key_from_bytes(key.public_key.as_slice()));
        promise.set_value(tonlib_api::make_object<tonlib_api::key>(key_bytes.serialize(true),
                                                                   std::move(key.secret)));
        return td::Status::OK();
    ...
    td::Status TonlibClient::do_request(const tonlib_api::changeLocalPassword &request,
        ...
        TRY_RESULT(input_key, from_tonlib(*request.input_key_));
        TRY_RESULT(key, key_storage_.change_local_password(std::move(input_key),
                                                           std::move(request.new_local_password_)));
        promise.set_value(tonlib_api::make_object<tonlib_api::key>(key.public_key.as_slice().str(),
                                                                   std::move(key.secret)));
        return td::Status::OK();
    }
    ...
    tonlib_api::object_ptr<tonlib_api::Object>
    TonlibClient::do_static_request(tonlib_api::setLogStream &request) {
    ...
    tonlib_api::object_ptr<tonlib_api::Object>
    TonlibClient::do_static_request(const tonlib_api::getLogStream &request) {
    ...
    tonlib_api::object_ptr<tonlib_api::Object> TonlibClient::do_static_request(
        ...
        auto result = Logging::set_tag_verbosity_level(request.tag_,
                                                       static_cast<int>(request.new_verbosity_level_));
        if (result.is_ok()) {
            return tonlib_api::make_object<tonlib_api::ok>();
        } else {
            return tonlib_api::make_object<tonlib_api::error>(400, result.message().str());
    ...
    tonlib_api::object_ptr<tonlib_api::Object> TonlibClient::do_static_request(
        ...
        return tonlib_api::make_object<tonlib_api::logVerbosityLevel>(
                Logging::get_verbosity_level());
    }
    ...
    tonlib_api::object_ptr<tonlib_api::Object>
    TonlibClient::do_static_request(const tonlib_api::getLogTags &request) {
        return tonlib_api::make_object<tonlib_api::logTags>(Logging::get_tags());
    }

    tonlib_api::object_ptr<tonlib_api::Object>
    TonlibClient::do_static_request(const tonlib_api::addLogMessage &request) {
        Logging::add_message(request.verbosity_level_, request.text_);
        return tonlib_api::make_object<tonlib_api::ok>();
    }

    tonlib_api::object_ptr<tonlib_api::Object>
    TonlibClient::do_static_request(const tonlib_api::encrypt &request) {
        return tonlib_api::make_object<tonlib_api::data>(
                SimpleEncryption::encrypt_data(request.decrypted_data_, request.secret_));
    }

    tonlib_api::object_ptr<tonlib_api::Object>
    TonlibClient::do_static_request(const tonlib_api::decrypt &request) {
        ...
        if (r_data.is_ok()) {
            ...
            return status_to_tonlib_api(
                    r_data.error().move_as_error_prefix(TonlibError::KeyDecrypt()));
        }
    }

    tonlib_api::object_ptr<tonlib_api::Object>
    TonlibClient::do_static_request(const tonlib_api::kdf &request) {
        auto max_iterations = 10000000;
        if (request.iterations_ < 0 || request.iterations_ > max_iterations) {
            return status_to_tonlib_api(
                    TonlibError::InvalidField("iterations", PSLICE() << "must be between 0 and "
                                                                     << max_iterations));
        }
        return tonlib_api::make_object<tonlib_api::data>(
    ...
    td::Status TonlibClient::do_request(int_api::GetAccountState request,
        ...
        actors_[actor_id] = td::actor::create_actor<GetRawAccountState>(
                "GetAccountState", client_.get_client(), request.address,
                std::move(request.block_id),
                actor_shared(this, actor_id),
                promise.wrap([address = request.address, wallet_id = wallet_id_,
                                     o_public_key = std::move(request.public_key)](
                        auto &&state) mutable {
                    auto res = td::make_unique<AccountState>(std::move(address), std::move(state),
                                                             wallet_id);
    ...
    td::Status TonlibClient::do_request(int_api::RemoteRunSmcMethod request,
        ...
        actors_[actor_id] = td::actor::create_actor<RemoteRunSmcMethod>(
                "RemoteRunSmcMethod", client_.get_client(), std::move(request),
                actor_shared(this, actor_id), std::move(promise));
        return td::Status::OK();
    }

    td::Status TonlibClient::do_request(int_api::GetPrivateKey request,
                                        td::Promise<KeyStorage::PrivateKey> &&promise) {
        TRY_RESULT(pk, key_storage_.load_private_key(std::move(request.input_key)));
        promise.set_value(std::move(pk));
        return td::Status::OK();
    }

    td::Status TonlibClient::do_request(int_api::GetDnsResolver request,
                                        td::Promise<block::StdAddress> &&promise) {
        client_.with_last_config(
                promise.wrap([](auto &&state) mutable -> td::Result<block::StdAddress> {
    ...
    td::Status
    TonlibClient::do_request(int_api::SendMessage request, td::Promise<td::Unit> &&promise) {
        client_.send_query(ton::lite_api::liteServer_sendMessage(
                vm::std_boc_serialize(request.message).move_as_ok()),
                           to_any_promise(std::move(promise)));
        return td::Status::OK();
    }
    ...
    td::Status TonlibClient::do_request(const tonlib_api::liteServer_getInfo &request,
        ...
        client_.send_query(ton::lite_api::liteServer_getVersion(), promise.wrap([](auto &&version) {
            return tonlib_api::make_object<tonlib_api::liteServer_info>(version->now_,
                                                                        version->version_,
    ...
    td::Status TonlibClient::do_request(tonlib_api::withBlock &request,
        ...
        ton::BlockIdExt block_id(request.id_->workchain_, request.id_->shard_, request.id_->seqno_,
                                 root_hash, file_hash);
